...
export default function FundingApplicationPage() {
  ...
  useEffect(() => {
    const initMarketplace = async () => {
      try {
        const res = await fetch("/api/init/marketplace", { method: "POST" });
        const result = await res.json();
        console.log("Marketplace init result:", result);
      } catch (error) {
  ...
  if (isSubmitted) return <SuccessScreen formData={formData} currentTime={currentTime} />;
  ...
  return (
    <div className="min-h-screen py-24 container mx-auto px-6">
      ...
      <h1 className="text-4xl md:text-5xl font-bold mb-6">
        <span className="gradient-text">Funding</span> <span>Application</span>
      </h1>
      <p className="text-gray-300 mb-10 max-w-2xl">
        This page only initializes the marketplace on load.
      </p>
